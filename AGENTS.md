...
- The Allium checker for `docs/specs/` as a project-managed binary: pinned by
  version and SHA-256 inside the `biscuit-games-tooling` package that
  `pyproject.toml` pins, installed into the gitignored `.tools/bin/` by
  `just install-allium`, and run by `just check-specs` and `just analyse-specs`,
  which read the reported JSON rather than the exit code neither subcommand
  makes trustworthy. Both are hooks in the read-only gate and recipes inside
  `just check`, so a fresh worktree needs `just install-allium` before the
  gate passes. See
