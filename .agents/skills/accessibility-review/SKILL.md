...
2. Check the colour obligation. Correct and present each carry a marker bar and absent deliberately carries none — the absence beside a dimmed letter is the indication — and every letter result and key state has an accessible description, so the board is readable without colour vision.
