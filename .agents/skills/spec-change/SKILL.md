...
1. Read `AGENTS.md` and `docs/explanation/specifications.md`. Identify which module owns the behaviour: `words`, `game`, `settings`, `statistics`, `sharing` or `daily`.
...
4. Check that dependent modules still hold. `game.allium` is depended on by the other four, so a change to a trigger or an entity ripples.
