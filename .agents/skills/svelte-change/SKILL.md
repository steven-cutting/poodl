...
2. Inspect the route, the component, the ports under `src/lib/ports/` and the two `@steven-cutting/biscuit-games` exports — the device's preferences and its keyboard — and the existing tests before editing. Never reach for `localStorage`, `crypto`, `Date`, or `navigator.clipboard` outside a port adapter.
