...
5. Record the provenance and licence of any list you bring in. These files ship inside the published site.
6. Never hand-edit a word to satisfy `typos`; the data directory is excluded from it in `pyproject.toml` precisely because a dictionary looks like a list of misspellings.
7. Run `just frontend-unit` — `tests/words.test.ts` asserts every obligation above — then `just check`.
