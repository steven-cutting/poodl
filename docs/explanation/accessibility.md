...
dead under exactly the finger the rule exists for, so a pressed key draws a ring in the
...
them owns it. Settings is the exception worth naming: nine native radios and checkboxes
...
is taken from Chromium by the stories instead.
...
- `-webkit-touch-callout` is declared and never verified. jsdom's parser drops it, desktop
  Chromium does not report it, and the platform it is for is iOS Safari.
- `:active` is a state only real input produces. No synthetic event reaches it and no play
  ...
  drawn in resolve to real, different colours. That the ring is legible on a plain key and
  on each mark was checked by eye in both palettes and in high contrast, and the contrast
  figures above were computed rather than eyeballed — but no gate holds either.
