...
1. **The genre was named.** Both deleted drafts, and the first nine candidates of this one,
   ...
   broke the sticker prior in one round. That paragraph is the top of
...
2. **Geometry was given, not asked for.** `armature/front.svg` and `armature/profile.svg`
   ...
   diagram disagree the diagram wins. The profile armature exists because the first six
   profile candidates all drew the long tapering show-poodle wedge the bible names as the
   second most common failure; with the armature attached, showing the skull as a circle and
   the muzzle as a stub three tenths of its diameter, the wedge disappeared in one round.
3. **One canon, then everything from it.** The front view was generated first and gated on
   ...
   all thirty-two later generations, so every panel is the same dog rather than thirteen
   readings of a brief.

Thirty-eight images were generated in all. The chosen one for each panel is named in
`prompts/chosen-panels.txt`; the rest were discarded.
...
- **Snapped.** The model lands within about eight units of every token and never on one, and
  ...
  the result resampled, so the fills are exactly the bible's values and nothing else appears.
